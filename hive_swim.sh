...
elif [ $(echo "$temp_water >= 15" | bc) -eq 1 ] \
	&& [ $(echo "$temp_water < 22" | bc) -eq 1 ]; then
	echo "${GREEN}What are you waiting for?"
elif [ $(echo "$temp_water > 22" | bc) -eq 1 ] \
	&& [ $(echo "$temp_water <= 30" | bc) -eq 1 ]; then
	echo "${YELLOW}Err what? Somebody left the hot tap on."
elif [ $(echo "$temp_water > 30" | bc) -eq 1 ]; then
